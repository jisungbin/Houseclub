...
class ClubhouseErrorResponse : ErrorResponse {
    ...
    override fun bindErrorView(view: View) {
        val txt: TextView = view.findViewById<TextView>(R.id.error_text)
        if (BuildConfig.DEBUG) txt.setText("""
    ...
    override fun showToast(context: Context) {
        if (BuildConfig.DEBUG) Toast.makeText(context, message, Toast.LENGTH_LONG)
            .show() else Toast.makeText(context, R.string.error_loading, Toast.LENGTH_SHORT).show()
    }
}