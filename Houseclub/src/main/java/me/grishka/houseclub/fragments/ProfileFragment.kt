...
class ProfileFragment : LoaderFragment() {
    ...
    override fun onCreateContentView(
        ...
        if (self) {
            ...
            photo.setOnClickListener(View.OnClickListener { v: View -> onPhotoClick(v) })
        }
        return v
    }
        ...
        currentRequest = GetProfile(arguments.getInt("id"))
            .setCallback(object : SimpleCallback<GetProfile.Response?>(this) {
                override fun onSuccess(result: GetProfile.Response) {
                    ...
                    if (user!!.photoUrl != null) ViewImageLoader.load(photo,
                        d,
                        user!!.photoUrl) else photo!!.setImageDrawable(d)
                    followsYou!!.visibility = if (user!!.followsMe) View.VISIBLE else View.GONE
                    followers!!.text = resources.getQuantityString(R.plurals.followers,
                        user!!.numFollowers,
                        user!!.numFollowers)
                    following!!.text = resources.getQuantityString(R.plurals.following,
                        user!!.numFollowing,
                        user!!.numFollowing)
                    bio!!.text = user!!.bio
                    ...
                    if (user!!.twitter == null && user!!.instagram == null) {
                        ...
                        twitter!!.visibility =
                            if (user!!.twitter == null) View.GONE else View.VISIBLE
                        instagram!!.visibility =
                            if (user!!.instagram == null) View.GONE else View.VISIBLE
                        if (user!!.twitter != null) twitter!!.text = user!!.twitter
                        if (user!!.instagram != null) instagram!!.text = user!!.instagram
                    }
                    var joined =
                        getString(R.string.joined_date, DateFormat.getDateInstance().format(
                            user!!.timeCreated))
                    if (user!!.invitedByUserProfile != null) {
                        ...
                        if (user!!.invitedByUserProfile!!.photoUrl != null) ViewImageLoader.load(
                            inviterPhoto,
                            d2,
                            user!!.invitedByUserProfile!!.photoUrl) else inviterPhoto!!.setImageDrawable(
                            d2)
                    } else {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (VoiceService.Companion.getInstance() != null) {
    ...
    private fun onInstagramClick(v: View) {
        startActivity(Intent(Intent.ACTION_VIEW,
            Uri.parse("https://instagram.com/" + user!!.instagram)))
    }
            ...
            .setPositiveButton(R.string.save) { dialogInterface, i ->
                ...
                UpdateBio(newBio)
                    ...
                    .setCallback(object : Callback<BaseResponse?> {
                        override fun onSuccess(result: BaseResponse?) {
                            user!!.bio = newBio
                            if (TextUtils.isEmpty(newBio)) bio!!.setText(R.string.update_bio) else bio!!.text =
                                newBio
                        }
    ...
    companion object {
        private const val PICK_PHOTO_RESULT = 468
    }
}