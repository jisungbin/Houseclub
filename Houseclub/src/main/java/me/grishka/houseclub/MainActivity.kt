                    ...
                    .setCallback(object : Callback<CheckWaitlistStatus.Response> {
                        override fun onSuccess(result: CheckWaitlistStatus.Response) {
                            if (!result.isWaitlisted) {
                                ...
                                if (result.isOnboarding) showFragmentClearingBackStack(
                                    RegisterFragment()) else showFragmentClearingBackStack(
                                    HomeFragment())
...
//									}
    ...
    fun joinChannel(chan: Channel) {
        if (VoiceService.Companion.getInstance() != null) {
            val current: Channel = VoiceService.Companion.getInstance().getChannel()
